...
def test_no_anomaly_below_threshold():
    for _ in range(anomaly_service.ANOMALY_THRESHOLD - 1):
        anomaly_service.record_report("AA-1111", "Megenagna", "Meg-Bole 01", "Route Chopping")
...
def test_anomaly_created_at_threshold():
    for _ in range(anomaly_service.ANOMALY_THRESHOLD):
        anomaly_service.record_report("AA-2222", "Megenagna", "Meg-Bole 01", "Route Chopping")
...
def test_report_count_accumulates_after_anomaly_created():
    plate = "AA-3333"
    for _ in range(anomaly_service.ANOMALY_THRESHOLD + 1):
        anomaly_service.record_report(plate, "Megenagna", "Meg-Bole 01", "Route Chopping")
...
def test_spam_filter_discards_over_ceiling():
    plate = "AA-4444"
    results = [
        anomaly_service.record_report(plate, "Megenagna", "Meg-Bole 01", "Fare Overcharge")
...
def test_update_status_forward_only():
    ...
    success, _ = anomaly_service.update_status(anomaly_id, "Reviewed", "", updated_by_user_id=1)
    assert success is True

    success2, error2 = anomaly_service.update_status(anomaly_id, "Resolved", "RCP-2026-001", updated_by_user_id=1)
...
def test_update_status_rejects_bad_receipt_format():
    ...
    success, _ = anomaly_service.update_status(anomaly_id, "Resolved", "not-a-receipt", updated_by_user_id=1)
